...
void CSound::FreeChannel()
{
	...
	if (m_pSound->Loaded())
	{
		...
		{
		case btSTREAM:
			FSOUND_Stream_Stop(m_pSound->GetStream());
			m_PlayingChannels--;
			break;
		case btSAMPLE:
			FSOUND_StopSound(m_nChannel);
			m_PlayingChannels--;
			break;
		}
	}
...
void CSound::SetMinMaxDistance(float fMinDist, float fMaxDist)
{
	...
	{
		GUARD_HEAP;
		FSOUND_Sample_SetMinMaxDistance(m_pSound->GetSample(), fMinDist, CRYSOUND_MAXDIST);//fMaxDist);
	}
}
...
void CSound::SetLoopPoints(const int iLoopStart, const int iLoopEnd)
{
	...
	{
		GUARD_HEAP;
		FSOUND_Sample_SetLoopPoints(m_pSound->GetSample(), iLoopStart, iLoopEnd);
	}
}
