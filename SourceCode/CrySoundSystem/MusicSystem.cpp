 ...
 */
 //signed char __cdecl _StreamingCallback(FSOUND_STREAM *pStream, void *pBuffer, int nLength, INT_PTR nParam)
signed char F_CALLBACKAPI _StreamingCallback(FSOUND_STREAM* pStream, void* pBuffer, int nLength, void* nParam)
{
	if (!nParam)
		return FALSE;
...
bool CMusicSystem::Init()
{
	...
	m_pStream = FSOUND_Stream_Create(_StreamingCallback, (int)((float)m_nSampleRate * m_fLatency) * m_nBytesPerSample, FSOUND_STEREO | FSOUND_16BITS | FSOUND_SIGNED | FSOUND_2D, 44100, this);
	if (!m_pStream)
		return false;;
...
bool CMusicSystem::StartPlaying()
{
	...
	m_nChannel = FSOUND_Stream_Play(FSOUND_FREE, m_pStream);
	if (m_nChannel < 0)
	{
		FlushPatterns();
		return false;
	}
	FSOUND_SetVolume(m_nChannel, (int)(m_fMasterVolume * 255.0f));
	return true;
}
...
*/
signed char CMusicSystem::StreamingCallback(FSOUND_STREAM* pStream, void* pBuffer, int nLength)
{
	CSmartCriticalSection SmartCriticalSection(m_CS);
