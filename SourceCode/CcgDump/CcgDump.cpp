...
void main(int argc, char* argv[])
{
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		else
		{
			...
			else if (!strcmpi(szOption, "meshIndices"))
			{
				g_bDumpMeshIndices = true;
			}
			...
			else if (!strcmpi(szOption, "indexMaps"))
			{
				g_bDumpIndexMaps = true;
			}
			...
			else if (!strcmpi(szOption, "anims") || !strcmpi(szOption, "anim"))
			{
				g_bDumpAnims = true;
			}
			else if (!strcmpi(szOption, "briefMaterials") || !strcmpi(szOption, "briefMtls") || !strcmpi(szOption, "briefMtl"))
			{
