...
void printChunkGIExtUVs(const char* pData, unsigned nSize)
{
	...
	for (i = 0; i < g_GI.numExtTangents; ++i)
	{
		...
		uvMin.u = crymin(uvMin.u, pUVs[i].u);
		uvMin.v = crymin(uvMin.v, pUVs[i].v);
		uvMax.u = crymax(uvMax.u, pUVs[i].u);
		uvMax.v = crymax(uvMax.v, pUVs[i].v);
	}
...
void printChunkGeometryInfo(const char* pData, unsigned nSize)
{
	...
	{
		CRYASSERT((const char*)Reader.GetData() + Reader.GetDataSize() <= pDataEnd);
	...
	CRYASSERT(pParsedDataEnd <= pDataEnd);
