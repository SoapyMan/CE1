...
{
	CRYASSERT(x == 0);		// sonst macht das glaub ich keinen Sinn
	p[0] = x; p[1] = x; p[2] = x;
}
...
public:
	...
	void GatherRayHitsDirection(const CVector3D _invStart, const CVector3D _invDir, Sink& inSink, const float infRayMax = FLT_MAX)
	{
		...
		CRYASSERT(vPosInCube.p[0] > 0);
		CRYASSERT(vPosInCube.p[1] > 0);
		CRYASSERT(vPosInCube.p[2] > 0);
		CRYASSERT(vPosInCube.p[0] <= 1.0f);
		CRYASSERT(vPosInCube.p[1] <= 1.0f);
		CRYASSERT(vPosInCube.p[2] <= 1.0f);
